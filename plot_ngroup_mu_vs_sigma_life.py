...
data = np.genfromtxt(outputfile, skiprows=1,usecols=(1,2,5,6))
...
mean_group = data[:,2].reshape(nmu,nsigma).transpose()
sd_group = data[:,3].reshape(nmu,nsigma).transpose()

mean_group = ndimage.filters.gaussian_filter(mean_group,1.0)
sd_group = ndimage.filters.gaussian_filter(100.0*sd_group/mean_group,1.0)
...
ax.set_xlim(0.0,4.0)
ax.set_ylim(0.1,1.0)
plt.pcolor(mu_life,sigma_life, mean_group,vmin=1, vmax = 11)
...
fig1.savefig('meangroup_mu_vs_sigma.png', format='png')
...
ax.set_xlim(0.0,4.0)
ax.set_ylim(0.1,1.0)
plt.pcolor(mu_life,sigma_life, sd_group)
#plt.hexbin(mu_life,sigma_life,C=mean_group, gridsize = 100)    
cb = plt.colorbar()
cb.set_label('Standard Error of Mean Group Number (%)', fontsize=20)
#cs = ax.contour(mu_life_values,sigma_life_values, mean_group_values.transpose(), levels=contourlevels, colors='k')
#ax.clabel(cs,contourlevels)
   
fig1.savefig('sdgroup_mu_vs_sigma.png', format='png')
plt.show()
